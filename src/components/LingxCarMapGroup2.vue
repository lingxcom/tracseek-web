...
function sendDelTids(arr){
	...
	for(let i=0;i<arr.length;i++){
		...
		if(deviceIdString.length>0)deviceIdString=deviceIdString.substring(0,deviceIdString.length-1);
	websocketsend({cmd:1006,car_id:deviceIdString,language:language});
}


watch(()=>props.tids,function(newValue,oldValue){
	//let arr=newValue;
	sendDelTids(oldValue);
	sendTids(newValue);
});
</script>

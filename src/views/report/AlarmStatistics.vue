...
<script setup>
  ...
  if(p2){
	...
	function searchGrid(){
		...
		api.callApi({apicode:1125,tids:tids.value,gids:gids.value,stime:sdate.value,etime:edate.value},function(json){
		  tableData.value=json.data;
		  for(let i=0;i<json.types.length;i++){
			  fields.value.push({name:json.types[i],code:json.types[i]});
		  }
		})
	}
	function day7(){

<template>
	<layout2>
		...
		<el-container style="width: 100%;height: 100%;margin: 0px;padding: 0px;background-color: #dfe9f6;">
		...
		<el-button-group  style="margin-left: 5px;" size="small">
		 <el-button @click="month3"  plain  >{{t('el.report.shortcuts.time201')}}</el-button>
		 <el-button @click="month6"  plain >{{t('el.report.shortcuts.time202')}}</el-button>
		 <el-button @click="month12"  plain >{{t('el.report.shortcuts.time203')}}</el-button>
		</el-button-group>
		                 </div>
		             </el-header>
...
<script setup>
  ...
  if(p2){
	...
	function searchGrid(){
		...
		api.callApi({apicode:1122,tids:tids.value,gids:gids.value,stime:sdate.value,etime:edate.value},function(json){

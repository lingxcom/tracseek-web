...
<script setup>
	...
	function checkChange(data,aaa,bbb){
		var param={apicode:9018,checked:aaa,checkedSub:bbb,menu_id:data.id,role_id:props.id};
		if(isInit)
	...
	onMounted(function(){
		emit("setype",2);
		emit("setitle",t('el.role.menuauth'));
		var param={apicode:9017,fid:0,role_id:props.id};
        api.callApi(param,function(json){

<template>
	...
	<el-tree ref="tree1"  :props="defaultProps"  style="background-color: #fff;" :check-strictly="true" 
...
<script setup>
	...
	function checkChange(data,aaa,bbb){
		var param={apicode:1104,checked:aaa,checkedSub:bbb,group_id:data.id,user_id:props.id};
		if(isInit)
	...
	onMounted(function(){
		emit("setype",2);
		emit("setitle",t('el.user.deviceauth'));
		var param={apicode:1103,fid:0,user_id:props.id};
        api.callApi(param,function(json){

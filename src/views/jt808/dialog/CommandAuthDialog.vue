...
<script setup>
	...
	function checkChange(data,aaa,bbb){
		var param={apicode:1107,checked:aaa,checkedSub:bbb,cmd_id:data.id,user_id:props.id};
		if(isInit)
	...
	onMounted(function(){
		emit("setype",2);
		emit("setitle",t('el.user.cmdauth'));
		var param={apicode:1106,fid:0,user_id:props.id};
        api.callApi(param,function(json){
